...
namespace AnalisisNumerico2024
{
    public partial class Form1 : Form
    {
        ...
        double CalcularXrAbierto(string metodo, string funcion, double xi, double xd)//Preguntar
        {
        ...
        double CalcularXrCerrado(string metodo, string funcion, double xi, double xd, double error)
        {
            double derivada, tolerancia = 0;
            double xr = 0;
            switch (metodo)
                ...
                if (analizadorFuncion.Sintaxis(txtFuncion.Text, 'x')) //Evaluo sintaxis correcta
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                xr = CalcularXrAbierto(metodoSeleccionado, txtFuncion.Text, xi, xd);
            ...
            private void CalcularTangenteOSecante()
            {
                ...
                if (analizadorFuncion.Sintaxis(txtFuncion.Text, 'x')) //Evaluo sintaxis correcta
                {
                    ...
                    for (int i = 1; i <= iteraciones; i++)
                    {
                        xd = analizadorFuncion.EvaluaFx(xi); //Preguntarrrr
                        xr = CalcularXrCerrado(metodoSeleccionado, txtFuncion.Text, xi, xd, error);
                        if (double.IsNaN(xr))
                        {
                            txtRaiz.Text = "No se encuentra";
                            txtConverge.Text = "No, diverge";
                            txtCantIteraciones.Text = iteraciones.ToString();
                            txtErrorRelativ.Text = error.ToString();
                            break;
                        }
                        error = Math.Abs((xr - xrAnterior) / xr);
